...
#     series::S
# end
struct ChebyshevCluster{T, N, I, J} <: AbstractChebyshevSeries{T, N}
    cseries::NTuple{I, ChebyshevSeries{T, N}}
    tseries::NTuple{J, TransformedChebyshevSeries{T, N}}
end
...
# end
function ChebyshevCluster(series::AbstractChebyshevSeries{T, N}...) where {T, N}
    cseries_vec = Vector{ChebyshevSeries}()
    tseries_vec = Vector{TransformedChebyshevSeries}()
    for s in series
        if s isa ChebyshevSeries
            push!(cseries_vec, s)
        elseif s isa TransformedChebyshevSeries
            push!(tseries_vec, s)
        else
            throw(ArgumentError("Invalid input type: $(typeof(s))"))
        end
    end
    
    I = length(cseries_vec)
    J = length(tseries_vec)

    cseries = NTuple{I, ChebyshevSeries}(cseries_vec)
    tseries = NTuple{J, TransformedChebyshevSeries}(tseries_vec)
    
    return ChebyshevCluster{T, N, I, J}(cseries, tseries)
end
...
function contains(h::ChebyshevCluster{T, N, I, J}, x::SVector{N, T}) where {T, N, I, J}
    
    cseries = h.cseries
    for i in 1:I
        if contains(cseries[i], x)
            return 1, i
        end
    end
